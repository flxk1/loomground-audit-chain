...
Keys live under `~/.workspace/keys/<host_id>/` (`WORKSPACE_KEY_DIR` overrides). The log root defaults to `~/.workspace/log` (`RVND_LOG_ROOT` or `log_root=` overrides).
...
- host ports: `mutation_log.purged_pair_ref`, `content_signature`, `register_workspace`, `witness_escape.quarantine`; compatibility constants in [docs/seam.md](docs/seam.md)

## Family

Assurance artifacts, pillar: `intact` of [governance-certification](https://github.com/flxk1/governance-certification). Consumes `loomground-workspace` (folder identity, log root, allowlist). Consumed by hosts, e.g. RVND; optional for every consumer. Catalogue: [loomground/CATALOGUE.md](https://github.com/flxk1/loomground/blob/main/CATALOGUE.md).
